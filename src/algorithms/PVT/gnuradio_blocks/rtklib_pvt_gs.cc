...
rtklib_pvt_gs::rtklib_pvt_gs(uint32_t nchannels,
    ...
    : gr::sync_block("rtklib_pvt_gs",
      ...
      d_compute_rate_ms(conf_.compute_rate_ms),
            ...
            if (d_gnss_observables_map.empty() == false)
                {
                                            ...
                                            if (!d_gnss_observables_map_t0.empty() && !d_gnss_observables_map_t1.empty())
                                                {
                                                    ...
                                                    if (current_RX_time_ms % d_compute_rate_ms == 0)
                                                        {
                    ...
                    if (flag_compute_pvt == true && d_enable_rx_clock_correction == true)
                        {
                                    ...
                                    if (d_first_fix == true)
                                        {
                                            ...
                                            ss << " is Lat DATA EXPUNGED [deg], Long = DATA EXPUNGED"
                    ...
                    // PVT MONITOR
                    if (d_user_pvt_solver->is_valid_position() && flag_compute_pvt_output)
                        {
